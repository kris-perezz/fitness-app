 ...
 *
 * Wider than `PAGE` at `lg`, and Profile is not the reason to hold it back --
 * that screen is `PAGE`, which stops at `md` and never reaches this cap. The
 * calendar column is a fixed 22rem, so every pixel added here goes to the
 * chart beside it, which is the half that gets better with room.
 *
...
export const PAGE_SPLIT =
  "mx-auto w-full max-w-md flex-1 space-y-2 px-2 pb-[calc(6rem+env(safe-area-inset-bottom))] pt-1 md:max-w-3xl md:px-6 md:pb-10 md:pt-8 lg:grid lg:max-w-6xl lg:grid-cols-[minmax(0,1fr)_22rem] lg:content-start lg:items-start lg:gap-6 lg:space-y-0";
