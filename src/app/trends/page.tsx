...
export default async function TrendsPage() {
  ...
  const [{ data: days }, { data: goals }, top] = await Promise.all([
    ...
    supabase
      ...
      .select("calorie_goal, protein_goal_g, strict_mode")
      .maybeSingle(),
  ...
  return (
    <TrendsScreen
      ...
      // Calm passes null for the same reason it passes null for protein -- the
      // no-goal path was already here, so neither chart needed a second mode.
      calorieGoal={
        goals?.strict_mode === true ? ((goals?.calorie_goal as number | undefined) ?? null) : null
      }
      // S79. THE PROTEIN FLOOR IS A STRICT-MODE IDEA, here as much as on the log
      // tab: a line across the chart turns thirty days of eating into thirty
      // days scored against a number, and thirty bars under a line read as
      // thirty small failures in a way one bar never does. Calm reuses the
      // no-goal path that already existed, so the chart simply has no line.
      proteinGoal={
        goals?.strict_mode === true ? ((goals?.protein_goal_g as number | undefined) ?? null) : null
      }
